...
date      = calibration.iloc[session].loc['Date']
electrode = calibration.iloc[session].loc['Electrode']
logger.info(f'Now processing - Date: {date}; Electrode: {electrode}.')
...
logger.info(f'Raw data loaded from {data_folder}')
...
logger.info(f'Raw data saved as Preprocess-{electrode}-{date}.png')
...
while int(input('Manual cleaning or not (0/1)? ')):
    ...
    if action.startswith('f'):
        ...
        logger.info(f'Manual cleaning run {run}: new starting point from {start}.')
    elif action.startswith('d'):
        data_mod.pop(run)
        conc = np.delete(conc, run)
        logger.info(f'Manual cleaning run {run}: drop run.')
    else:
        logger.warning('Invalid action, pick fix/drop.')
logger.info('Finish manual cleaning.')
...
if int(input('Sure to save (0/1)? ')):
    if data.shape[1] == concs.shape[0]: # The final check
        ...
        logger.info(f'Data saved in {electrode}_{date}_FSCV.npy and {electrode}_{date}_CONC.npy')
    else:
        logger.error('Sizes do not match.')
else:
    logger.warning('Data not saved. Check and restart.')
